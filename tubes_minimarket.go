...
var pembayaranList [maxPembayaran]Pembayaran

var jumlahBarang, jumlahTransaksi, jumlahPembayaran int
...
func main() {
	for {
		...
		fmt.Println("8. Catat Pembayaran")
...
func catatPembayaran() {
	if jumlahPembayaran >= maxPembayaran {
		fmt.Println("Tidak bisa mencatat pembayaran lagi, kapasitas penuh!")
		return
	}

	var pembayaran Pembayaran

	fmt.Print("Masukkan ID Transaksi yang dibayar: ")
	fmt.Scanln(&pembayaran.TransaksiID)

	index := findTransaksiByID(pembayaran.TransaksiID)
	if index == -1 {
		fmt.Println("Transaksi dengan ID tersebut tidak ditemukan")
		return
	}

	pembayaran.ID = jumlahPembayaran + 1
	pembayaran.TotalBayar = transaksiList[index].TotalHarga
	pembayaran.Tanggal = time.Now()
...
func lihatDaftarPembayaran() {
	...
	for i := 0; i < jumlahPembayaran; i++ {
		transaksi := transaksiList[findTransaksiByID(pembayaranList[i].TransaksiID)]
		fmt.Printf("%d. ID Pembayaran: %d, ID Transaksi: %d, Total Bayar: Rp %.2f, Tanggal: %s\n",
			i+1, pembayaranList[i].ID, pembayaranList[i].TransaksiID, pembayaranList[i].TotalBayar, pembayaranList[i].Tanggal.Format("02-01-2006 15:04:05"))
	}
}
